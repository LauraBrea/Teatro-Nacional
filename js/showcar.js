...
window.onload =  function showcar () {
    ...
    if (checkcar.lenght != 0) {
        ...
        for (const item of checkcar) {
            ...
            li.textContent = `${item.day} ${item.date} - ${item.show} - Boletos: ${item.qty} ${item.place} - Subtotal: $ ${item.subprice} - Descuento: -$ ${item.discount} - Total: $ ${item.pricetot}`;
            carlist.appendChild(li);
        }

        let fixed=document.getElementById ("fixedcost");
        fixed.textContent = `Cargo fijo por utilizar el servicio: +$ ${fixedCost}`;
